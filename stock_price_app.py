...
# File paths
price_file = 'NvidiaStockPrice.csv'
dividend_file = 'NvidiaStockDividend.csv'
split_file = 'NvidiaStockSplit.csv'
...
st.write("Nvidia Stock Price Data:")
st.dataframe(price_data.head())

st.write("Nvidia Stock Dividend Data:")
st.dataframe(dividend_data.head())

st.write("Nvidia Stock Split Data:")
st.dataframe(split_data.head())

# Merge datasets on the 'Date' column
merged_data = price_data.merge(dividend_data, on='Date', how='left').merge(split_data, on='Date', how='left')
...
if merged_data.isnull().sum().any():
    st.warning("Merged dataset contains missing values. Dropping missing values for simplicity.")
    merged_data = merged_data.dropna()

# Feature Engineering: Create additional features (e.g., moving average)
merged_data['Moving_Avg_5'] = merged_data['Close'].rolling(window=5).mean()
merged_data['Moving_Avg_10'] = merged_data['Close'].rolling(window=10).mean()
merged_data = merged_data.dropna()  
