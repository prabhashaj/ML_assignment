...
# Add Nvidia logo
st.image("nvidia.png", width=300)  # Ensure the image path is correct

# Load the dataset directly (instead of uploading)
file_path = 'NvidiaStockPrice.csv'  # Assuming the file is in the same directory
data = pd.read_csv(file_path)
...
# Future Predictions without random noise
st.subheader("Future Stock Price Predictions")
start_year = st.number_input("Start Year:", min_value=2024, max_value=2030, value=2024)
end_year = st.number_input("End Year:", min_value=2024, max_value=2030, value=2026)
...
if st.button("Predict Future Prices"):
    ...
    # Get last known values and repeat for future dates
    last_known_values = X.iloc[-1].to_frame().T  # Get the last known values as a DataFrame
    
    # Repeat the last known values for future predictions
    future_data = np.tile(last_known_values.values.flatten(), (len(future_dates), 1))
